...
static DATA: &'static str = include_str!("day25.txt");
...
pub fn main() {
    ...
    for a in 0.. {
        ...
        machine.regs[A] = a;
        machine.run(&mut instructions);
        if machine.output == "010101010101" {
            println!("{} => {}", a, machine.output);
            break;
        }
    }
}
